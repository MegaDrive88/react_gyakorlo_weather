...
export default class DataService{
    ...
    async getCurrentWeatherByLocation(lat: number, lon: number){
        return (await this.axios.get<any>(`forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,is_day,relative_humidity_2m,wind_speed_10m,wind_direction_10m&timezone=auto&`)).data
        // &temperature_unit=fahrenheit , rain,snowfall?
    }
    async getHourlyForecastByLocation(lat: number, lon: number){
        return (await this.axios.get<any>(`forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,weather_code,is_day,relative_humidity_2m,wind_speed_10m,wind_direction_10m&timezone=auto&forecast_days=2`)).data
    }
